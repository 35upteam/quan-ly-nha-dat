...
def get_data():
    try:
        ...
        # Mở file Google Sheet (ID của bạn)
        ...
        df.columns = df.columns.str.strip() # Xóa khoảng trắng thừa ở tiêu đề
        
        # Chuyển đổi cột Giá bán sang dạng số
...
with st.sidebar:
    ...
    is_admin = (admin_pass == "admin123") # Mật khẩu đăng nhập
...
if sheet_obj is None:
    st.error("Không thể kết nối với Google Sheet. Hãy kiểm tra lại Secrets hoặc ID Sheet.")
else:
    ...
    with tab_view:
        if df_raw.empty:
            st.info("Hiện chưa có căn hộ nào trong danh sách.")
        else:
            # Bộ lọc tìm kiếm
            ...
            with col3:
                price_f = st.slider("Giá bán (Tỷ)", 1.5, 12.0, (2.0, 6.0), step=0
