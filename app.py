...
# --- DIALOG CHI TIẾT (ĐƯA RA NGOÀI ĐỂ TRÁNH LỖI DUPLICATE) ---
    ...
    with cl1:
        ...
        if imgs and imgs[0]:
            ...
            if len(imgs) > 1:
                b1, b2 = st.columns(2)
                with b1:
                    if st.button("⬅️", key=f"prev_{mid}"):
                        st.session_state.ci -= 1
                        st.rerun()
                with b2:
                    if st.button("➡️", key=f"next_{mid}"):
                        st.session_state.ci += 1
                        st.rerun()
    ...
    with cl2:
        ...
        if is_adm:
            ...
            if not st.session_state[ck]:
                if st.button("✅ CHỐT CĂN", use_container_width=True, key=f"btn_{mid}"):
...
if sh_obj is not None and not df_raw.empty:
    ...
    def draw(df_in, ks):
        ...
        s_ma = ""
        if is_adm:
            s_ma = st.text_input("Mã căn (Admin)...", key=f"s{ks}").strip()
